...
func (tc *taskController) GetAllTasks(c echo.Context) error {
	c.Logger().Info("GetAllTasks called")
	...
	if err != nil {
		c.Logger().Errorf("Error getting all tasks: %v", err)
...
func (tc *taskController) GetTaskById(c echo.Context) error {
	c.Logger().Info("GetTaskById called")
	user := c.Get("user").(*jwt.Token)
	claims := user.Claims.(jwt.MapClaims)
	userId := claims["user_id"]
	id := c.Param("taskId")
	...
	if err != nil {
		c.Logger().Errorf("Error getting task by id: %v", err)
		return c.JSON(http.StatusInternalServerError, err.Error())
	}
...
func (tc *taskController) CreateTask(c echo.Context) error {
	c.Logger().Info("CreateTask called")
	user := c.Get("user").(*jwt.Token)
	claims := user.Claims.(jwt.MapClaims)
	userId := claims["user_id"]
	...
	if err := c.Bind(&task); err != nil {
		c.Logger().Errorf("Error binding task: %v", err)
		return c.JSON(http.StatusBadRequest, err.Error())
	}
	task.UserId = uint(userId.(float64))
	taskRes, err := tc.tu.CreateTask(task)
	if err != nil {
		c.Logger().Errorf("Error creating task: %v", err)
		return c.JSON(http.StatusInternalServerError, err.Error())
	}
...
func (tc *taskController) UpdateTask(c echo.Context) error {
	c.Logger().Info("UpdateTask called")
	user := c.Get("user").(*jwt.Token)
	claims := user.Claims.(jwt.MapClaims)
	userId := claims["user_id"]
	id := c.Param("taskId")
...
func (tc *taskController) DeleteTask(c echo.Context) error {
	c.Logger().Info("DeleteTask called")
	user := c.Get("user").(*jwt.Token)
	claims := user.Claims.(jwt.MapClaims)
	userId := claims["user_id"]
	id := c.Param("taskId")
